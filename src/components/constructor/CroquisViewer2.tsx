...
let _lastConteo: Record<string, number> | null = null;
  ...
  beds.forEach(bed => {
    ...
    if (modo === 'cama' && bed.type === 'litera') {
      ...
      if (topOcc || bottomOcc) {
        ...
        if (topOcc && bedColorMap?.[actualNumbers[0]]) {
          ctx.fillStyle = bedColorMap[actualNumbers[0]];
          ctx.fillRect(-w / 2, topY, w, topH);
        }
        if (bottomOcc && bedColorMap?.[actualNumbers[1]]) {
          ctx.fillStyle = bedColorMap[actualNumbers[1]];
          ctx.fillRect(-w / 2, bottomY, w, bottomH);
        }

        ctx.restore();
      }
    } else if (occupiedNumbers.length > 0 && bedColorMap?.[occupiedNumbers[0]]) {
      ctx.fillStyle = bedColorMap[occupiedNumbers[0]];
      ctx.beginPath();
      ctx.roundRect(-w / 2, -h / 2, w, h, 4);
      ctx.fill();
    }
...
const CroquisViewer2 = forwardRef<HTMLCanvasElement, CroquisViewer2Props>(function CroquisViewer2({ croquisData, carpaNombre, campamentoId, width = 700, height = 600, elementNumberOffset = 0, tipoContabilizacion = 'elemento', literasCount, individualesCount, duplexCount, disponiblesCarpa }, ref) {
  ...
  useEffect(() => {
    ...
    if (campamentoId === _lastCampId && _lastConteo) {
      setConteoActas(_lastConteo);
      return;
    }
    _lastCampId = campamentoId;
    contarActasPorRefugiado(campamentoId).then(data => {
      _lastConteo = data;
      setConteoActas(data);
    });
  }, [campamentoId]);
  ...
  const refugiadosDelCampamento = useMemo(
    () => refugiados.filter(r => r.campamento_id === campamentoId),
    [refugiados, campamentoId]
  );
  ...
  const bedColorMap = useMemo(() => {
    ...
    return map;
  }, [refugiadosDelCampamento, conteoActas]);
