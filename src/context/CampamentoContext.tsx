...
export function CampamentoProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    async function cargarDatos() {
      ...
      try {
        ...
        const refugiadosMapped: Refugiado[] = refRows.map(r => ({
          ...
          nacionalidad: (r.nacionalidad as string) || undefined,
          ...
          hogar_solidario: ((r.hogar_solidario as string || '').trim() || 'PRESENTE').toUpperCase(),
    ...
    const mapRefugiadoPayload = (r: Record<string, any>): Refugiado => ({
        ...
        hogar_solidario: ((r.hogar_solidario || '').trim() || 'PRESENTE').toUpperCase(),
  ...
  const agregarRefugiado = async (nuevo: Refugiado) => {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        nacionalidad: nuevo.nacionalidad || null,
        genero: nuevo.genero,
    ...
    const refugiadoCreado: Refugiado = {
      ...
      nacionalidad: data.nacionalidad as string | undefined,
  ...
  const actualizarRefugiado = async (id: string, actualizado: Refugiado): Promise<boolean> => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        nacionalidad: actualizado.nacionalidad || null,
