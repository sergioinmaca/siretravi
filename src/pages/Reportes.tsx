...
export default function Reportes() {
  ...
  // ── Exportar XLSX de Retirados ────────────────────────────────────────────
  const handleExportRetiradosXLSX = async () => {
    setIsGenerating(true);
    try {
      const nombreCamp = campamentoSeleccionado?.nombre || 'Campamento';
      const retirados = refugiadosDelCampamento.filter(r => esRetirado(r));
      await exportarRefugiadosXLSX(retirados, familiasDelCampamento, campamentoSeleccionado!.id, nombreCamp, 'retirados');
    } catch (err) {
      console.error('Error generando XLSX de retirados:', err);
    } finally {
      setIsGenerating(false);
    }
  };
  ...
  return (
    <div className="space-y-8 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        {puedeExportarIntegrantes && tienePermisoReporte('integrantes', campamentoSeleccionado?.id || '') && (
          <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-all flex flex-col justify-between min-h-[220px]">
            <div>
              ...
              <p className="text-sm text-slate-500 leading-relaxed">
                Exporta los registros de integrantes activos del campamento actual en formato Excel con las mismas columnas que el listado de integrantes.
              </p>
            </div>
            <div className="flex gap-4 mt-6 pt-4 border-t border-slate-50">
              <button
        ...
        {/* Card 6b: Reporte de Retirados */}
        {tieneAcceso && tienePermisoReporte('retirados', campamentoSeleccionado?.id || '') && (
          <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-all flex flex-col justify-between min-h-[220px]">
            <div>
              <h3 className="text-lg font-bold text-slate-800 mb-2">Reporte de Retirados</h3>
              <p className="text-sm text-slate-500 leading-relaxed">
                Listado de integrantes con estatus RETIRADO del campamento. Incluye datos completos con el mismo formato que el listado de integrantes activos.
              </p>
            </div>
            <div className="flex gap-4 mt-6 pt-4 border-t border-slate-50">
              <button
                onClick={handleExportRetiradosXLSX}
                disabled={!campamentoSeleccionado || isGenerating}
                className="flex-1 flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-xl font-medium text-sm transition-all disabled:opacity-50"
              >
                <FileDown size={18} />
